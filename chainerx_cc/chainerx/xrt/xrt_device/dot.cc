...
#include "chainerx/xrt/data_type.h"
...
void XrtDevice::Dot(const Array& a, const Array& b, const Array& out) {
    ...
    VisitDtype(out.dtype(), [&](auto pt) {
        ...
        for (int64_t i = 0; i < m; ++i) {
            for (int64_t l = 0; l < k; ++l) {
                int64_t a_i_l[] = {i, l};
                T a_value = xrt_internal::StorageToDataType<const T>(a_cast_iarray[a_i_l]);
                for (int64_t j = 0; j < n; ++j) {
                    int64_t acc_i_j[] = {i, j};
                    int64_t b_l_j[] = {l, j};
                    T b_value = xrt_internal::StorageToDataType<const T>(b_cast_iarray[b_l_j]);
                    AccT& acc_value = xrt_internal::StorageToDataType<AccT>(acc_iarray[acc_i_j]);
        ...
        if (!std::is_same<T, AccT>{}) {
            for (int64_t i = 0; i < m; ++i) {
                for (int64_t j = 0; j < n; ++j) {
                    int64_t i_j[] = {i, j};
                    AccT acc_value = xrt_internal::StorageToDataType<AccT>(acc_iarray[i_j]);
                    T& out_value = xrt_internal::StorageToDataType<T>(out_iarray[i_j]);

...
void XrtDevice::Fill(const Array& out, Scalar value) {
...
void XrtDevice::Arange(Scalar start, Scalar step, const Array& out) {
    VisitDtype(out.dtype(), [&](auto pt) {
        using T = typename decltype(pt)::type;
        struct Impl {
...
void XrtDevice::Identity(const Array& out) {
...
void XrtDevice::Eye(int64_t k, const Array& out) {
...
void XrtDevice::Diagflat(const Array& v, int64_t k, const Array& out) {
    ...
    VisitDtype(out.dtype(), [&](auto pt) {
        ...
        for (auto out_it = out_indexer.It(0); out_it; ++out_it) {
            out_iarray[out_it] = xrt_internal::DataToStorageType(T{0});
        }
...
void XrtDevice::Linspace(double start, double stop, const Array& out) {

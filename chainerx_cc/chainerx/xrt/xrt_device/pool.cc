...
#include "chainerx/xrt/col2im.h"
#include "chainerx/xrt/elementwise.h"
#include "chainerx/xrt/im2col.h"
#include "chainerx/xrt/tensor_dot.h"
...
class XrtMaxPoolForwardBackward : public chainerx::MaxPoolForwardBackward {
public:
    explicit XrtMaxPoolForwardBackward(
    ...
    Array Forward(const Array& x) override {
        ...
        col_ = xrt_internal::Im2Col(x, kernel_size_, stride_, pad_, cover_all_, GetLowestOrInf(x.dtype()));
    ...
    Array DoubleBackward(const Array& ggx) override {
        ...
        Array col = xrt_internal::Im2Col(ggx, kernel_size_, stride_, pad_, cover_all_, GetLowestOrInf(x_.dtype()));
        return Take(
...
std::unique_ptr<MaxPoolForwardBackward> XrtDevice::GetMaxPoolForwardBackward(
        const StackVector<int64_t, kMaxNdim>& kernel_size,
        const StackVector<int64_t, kMaxNdim>& stride,
        const StackVector<int64_t, kMaxNdim>& pad,
        bool cover_all) {
    return std::make_unique<XrtMaxPoolForwardBackward>(kernel_size, stride, pad, cover_all);
}
...
class XrtAveragePoolForwardBackward : public chainerx::AveragePoolForwardBackward {
public:
    explicit XrtAveragePoolForwardBackward(
            StackVector<int64_t, kMaxNdim> kernel_size,
            StackVector<int64_t, kMaxNdim> stride,
            StackVector<int64_t, kMaxNdim> pad,
    ...
    Array Forward(const Array& x) override {
        ...
        Array col = xrt_internal::Im2Col(x, kernel_size_, stride_, pad_, false, 0);
    ...
    Array Backward(const Array& gout) override {
        ...
        switch (pad_mode_) {
            case AveragePoolPadMode::kZero: {
                ...
                gx = xrt_internal::Col2Im(gcol, stride_, pad_, {x_.shape().begin() + 2, x_.shape().end()});
...
std::unique_ptr<AveragePoolForwardBackward> XrtDevice::GetAveragePoolForwardBackward(
        const StackVector<int64_t, kMaxNdim>& kernel_size,
        const StackVector<int64_t, kMaxNdim>& stride,
        const StackVector<int64_t, kMaxNdim>& pad,
        AveragePoolPadMode pad_mode) {
    return std::make_unique<XrtAveragePoolForwardBackward>(kernel_size, stride, pad, pad_mode);
}
